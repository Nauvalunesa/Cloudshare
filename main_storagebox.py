"""
============================================================
CLOUDSHARE - STORAGEBOX VERSION
============================================================

This is a modified version of main.py that stores encrypted files
on a remote storagebox via FTP/SFTP instead of local disk storage.

FEATURES:
- All uploaded files are stored on remote storagebox
- Encryption/decryption still happens on VPS
- Metadata (database) remains on local VPS
- Automatic retry on FTP failures
- Supports both SFTP (secure) and FTP protocols

SETUP:
1. Configure FTP credentials in environment variables or in code:
   - FTP_HOST: Your storagebox hostname
   - FTP_PORT: Port number (22 for SFTP, 21 for FTP)
   - FTP_USERNAME: Your storagebox username
   - FTP_PASSWORD: Your storagebox password
   - FTP_BASE_PATH: Base directory in storagebox

2. Run this file instead of main.py:
   uvicorn main_storagebox:app --host 0.0.0.0 --port 8000

REQUIREMENTS:
- paramiko (for SFTP support)
- All other requirements from main.py
...
# ============================================================
# STORAGEBOX FTP CONFIGURATION
# ============================================================
# Configure your storagebox FTP credentials here
FTP_HOST = os.getenv("FTP_HOST", "your.storagebox.host")  # e.g., "u123456.your-storagebox.de"
FTP_PORT = int(os.getenv("FTP_PORT", "22"))  # 22 for SFTP, 21 for FTP
FTP_USERNAME = os.getenv("FTP_USERNAME", "your-username")
FTP_PASSWORD = os.getenv("FTP_PASSWORD", "your-password")
FTP_BASE_PATH = os.getenv("FTP_BASE_PATH", "/storage")  # Base directory in storagebox
FTP_USE_SFTP = True  # True for SFTP (secure), False for regular FTP

# FTP Connection pool and timeout settings
FTP_TIMEOUT = 30  # Connection timeout in seconds
FTP_RETRY_ATTEMPTS = 3  # Number of retry attempts for failed operations
# ============================================================
...
# ============================================================
# STORAGEBOX FTP HELPER FUNCTIONS
# ============================================================

def ftp_connect():
    """Create and return FTP/SFTP connection to storagebox"""
    if FTP_USE_SFTP:
        # Use SFTP (secure)
        ...
        try:
            sftp.stat(FTP_BASE_PATH)
        except FileNotFoundError:
            # Create base path if not exists
            sftp.mkdir(FTP_BASE_PATH)

        return sftp, transport
    else:
        # Use regular FTP (not recommended for production)
...
def ftp_upload_file(filename: str, file_data: bytes) -> bool:
    """Upload file to storagebox via FTP/SFTP"""
    for attempt in range(FTP_RETRY_ATTEMPTS):
        try:
            if FTP_USE_SFTP:
            ...
            logger.error(f"FTP upload attempt {attempt + 1} failed: {e}")
            if attempt == FTP_RETRY_ATTEMPTS - 1:
                raise
            time.sleep(1)  # Wait before retry

    return False

def ftp_download_file(filename: str) -> bytes:
    """Download file from storagebox via FTP/SFTP"""
    for attempt in range(FTP_RETRY_ATTEMPTS):
        try:
            if FTP_USE_SFTP:
                sftp, transport = ftp_connect()
            ...
            logger.error(f"FTP download attempt {attempt + 1} failed: {e}")
            if attempt == FTP_RETRY_ATTEMPTS - 1:
                raise
            time.sleep(1)  # Wait before retry
...
def ftp_delete_file(filename: str) -> bool:
    """Delete file from storagebox via FTP/SFTP"""
    for attempt in range(FTP_RETRY_ATTEMPTS):
        try:
            if FTP_USE_SFTP:
                sftp, transport = ftp_connect()
            ...
            logger.error(f"FTP delete attempt {attempt + 1} failed: {e}")
            if attempt == FTP_RETRY_ATTEMPTS - 1:
                return False  # Don't raise on delete, just return False
            time.sleep(1)  # Wait before retry

    return False

def ftp_file_exists(filename: str) -> bool:
    """Check if file exists in storagebox"""
    try:
        if FTP_USE_SFTP:
            sftp, transport = ftp_connect()
            remote_path = f"{FTP_BASE_PATH}/{filename}"
        ...
        logger.error(f"FTP file exists check failed: {e}")
        return False
...
async def upload_file(request: Request, response: Response, file: UploadFile = File(...), filename: Optional[str] = Form(None), expire_value: Optional[int] = Form(None), expire_unit: Optional[str] = Form(None), upload_id: Optional[str] = Form(None)):
    try:
        ...
        # Check for duplicate filename in FTP storage
        counter = 1
        while ftp_file_exists(internal_name):
            final_name = f"{name}_{counter}{ext}"
            internal_name = f"{final_name}.enc"
            counter += 1

        # Upload to storagebox via FTP
        upload_progress[upload_id]["status"] = "uploading"
        try:
            ftp_upload_file(internal_name, encrypted_content)
        except Exception as e:
        ...
        uploaded_files[final_name] = {
            "path": internal_name,  # Store remote filename instead of local path
...
async def download_file(filename: str):
    try:
        ...
        remote_filename = data.get("path") if data else None
        if not data or not ftp_file_exists(remote_filename):
            raise HTTPException(status_code=404, detail="File not found")

        if data["expires_at"] and datetime.utcnow() > data["expires_at"]:
            # Delete from FTP storage
            try:
                ftp_delete_file(remote_filename)
            except:
                pass
        ...
        # Download from FTP storage
        try:
            encrypted_content = ftp_download_file(remote_filename)
        except Exception as e:
...
async def delete_file(filename: str, request: Request):
    ...
    try:
        ...
        remote_filename = file_data.get("path")
        ...
        if remote_filename:
            try:
                ...
                # Continue even if FTP delete fails
